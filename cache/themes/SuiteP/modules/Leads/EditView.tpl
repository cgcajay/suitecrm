...
{minify}
{capture name="label" assign="label"}{sugar_translate label='LBL_CITY_NAME' module='Leads'}{/capture}
{$label|strip_semicolon}:
...
<select name="{$fields.city_name_c.name}" 
id="{$fields.city_name_c.name}" 
title=''       
>
{if isset($fields.city_name_c.value) && $fields.city_name_c.value != ''}
{html_options options=$fields.city_name_c.options selected=$fields.city_name_c.value}
{else}
{html_options options=$fields.city_name_c.options selected=$fields.city_name_c.default}
{/if}
</select>
{else}
{assign var="field_options" value=$fields.city_name_c.options }
{capture name="field_val"}{$fields.city_name_c.value}{/capture}
{assign var="field_val" value=$smarty.capture.field_val}
{capture name="ac_key"}{$fields.city_name_c.name}{/capture}
{assign var="ac_key" value=$smarty.capture.ac_key}
<select style='display:none' name="{$fields.city_name_c.name}" 
id="{$fields.city_name_c.name}" 
title=''          
>
{if isset($fields.city_name_c.value) && $fields.city_name_c.value != ''}
...
<input
id="{$fields.city_name_c.name}-input"
name="{$fields.city_name_c.name}-input"
size="30"
value="{$field_val|lookup:$field_options}"
type="text" style="vertical-align: top;">
<span class="id-ff multiple">
<button type="button"><img src="{sugar_getimagepath file="id-ff-down.png"}" id="{$fields.city_name_c.name}-image"></button><button type="button"
id="btn-clear-{$fields.city_name_c.name}-input"
title="Clear"
onclick="SUGAR.clearRelateField(this.form, '{$fields.city_name_c.name}-input', '{$fields.city_name_c.name}');sync_{$fields.city_name_c.name}()"><span class="suitepicon suitepicon-action-clear"></span></button>
</span>
{literal}
<script>
	...
	{/literal}
			...
			{literal}
		(function (){
			var selectElem = document.getElementById("{/literal}{$fields.city_name_c.name}{literal}");
	...
	SUGAR.AutoComplete.{$ac_key}.inputNode = Y.one('#{$fields.city_name_c.name}-input');
	SUGAR.AutoComplete.{$ac_key}.inputImage = Y.one('#{$fields.city_name_c.name}-image');
	SUGAR.AutoComplete.{$ac_key}.inputHidden = Y.one('#{$fields.city_name_c.name}');
			...
			//global variable 
			sync_{/literal}{$fields.city_name_c.name}{literal} = function(){
				SyncToHidden();
			}
			function syncFromHiddenToWidget(){
				...
				for (i=0;i<selectElem.options.length;i++){
					...
					if (selectElem.options[i].value==selectElem.value && document.activeElement != document.getElementById('{/literal}{$fields.city_name_c.name}-input{literal}'))
						SUGAR.AutoComplete.{/literal}{$ac_key}{literal}.inputNode.set('value',selectElem.options[i].innerHTML);
				}
			}

            YAHOO.util.Event.onAvailable("{/literal}{$fields.city_name_c.name}{literal}", syncFromHiddenToWidget);
		{/literal}
...
addToValidate('EditView', 'city_name_c', 'enum', false,'{/literal}{sugar_translate label='LBL_CITY_NAME' module='Leads' for_js=true}{literal}' );
