...
{if !$fields.city_name_c.hidden}
{counter name="panelFieldCount" print=false}


{if is_string($fields.city_name_c.options)}
<input type="hidden" class="sugar_field" id="{$fields.city_name_c.name}" value="{ $fields.city_name_c.options }">
{ $fields.city_name_c.options }
{else}
<input type="hidden" class="sugar_field" id="{$fields.city_name_c.name}" value="{ $fields.city_name_c.value }">
{ $fields.city_name_c.options[$fields.city_name_c.value]}
{/if}
{/if}
...
{capture name="label" assign="label"}{sugar_translate label='LBL_SALUTATION' module='Leads'}{/capture}
{$label|strip_semicolon}:
</div>
